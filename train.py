...
from model.fss_network import FSSModel   
from util import dataset
from util.pseudo_dataset import DatasetSSL
...
def get_parser():
    ...
    parser.add_argument('opts', help='see config/coco/coco_split0_resnet50.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    model = FSSModel(layers=args.layers, shot=args.shot, \
    ...
    assert args.split in [0, 1, 2, 3, 5]
    train_transform = [
    ...
    if args.split == 5:
        train_data = DatasetSSL(cluster_num=args.cluster_num,
                                fold=args.split, shot=args.shot, data_root=args.data_root,
                                transform=train_transform, data_list=args.train_list, mode='train',
    ...
    Visualizer.initialize(args.debug_vis)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        filename = args.save_path + '/latest.pth'
        logger.info('Saving checkpoint to: ' + filename)
        torch.save({'epoch': epoch, 'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}, filename)

        print(args.evaluate, epoch)
        if args.evaluate and epoch>0:
            print('***************val*******************')
...
def train(train_loader, model, optimizer, transformer_optimizer, epoch, base_lrs):
    ...
    for i, (input, mask, s_input, s_mask, padding_mask, s_padding_mask) in enumerate(train_loader):
        ...
        output, loss = model(s_x=s_input, s_y=s_mask, x=input, y=mask, padding_mask=padding_mask, s_padding_mask=s_padding_mask)
        ...
        if Visualizer.visualize:
            Visualizer.visualize_prediction_batch(s_input, s_mask,
                                                  input, mask,
                                                  output, torch.tensor([0]), i,
                                                  torch.tensor(intersection[1]).float() / torch.tensor(union[1]).float())

...
def get_parser():
    ...
    parser.add_argument('opts', help='see config/coco/coco_split0_resnet50.yaml.yaml for all options', default=None, nargs=argparse.REMAINDER)
...
def validate(val_loader, model, criterion):
    ...
    for e in range(20):
        for i, (input, target, s_input, s_mask, padding_mask, s_padding_mask, subcls, ori_label) in enumerate(val_loader):
            ...
            mask = target.clone()
            pred = output.clone().max(1)[1]
            pred[padding_mask==255]=0
